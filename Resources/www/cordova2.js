...
require = function (id) {
    if (!modules[id]) {
        console.error('module ' + id + ' not found','cordovajs');
        throw 'module ' + id + ' not found';
    } else if (id in inProgressModules) {
        var cycle = requireStack.slice(inProgressModules[id]).join('->') + '->' + id;
        console.error('Cycle in require graph: ' + cycle,'cordovajs');
...
define = function (id, factory) {
        if (Object.prototype.hasOwnProperty.call(modules, id)) {
            console.error('module ' + id + ' already defined','cordovajs');
...
var cordova = {
            ...
            if (bNoDetach) {
                ...
                setTimeout(function () {
                    ...
                    if (type === 'deviceready') {
                        ...
                        console.debug('onDeviceReady','cordovajs');
                    }
                    documentEventHandlers[type].fire(evt);
                }, 0);
            }
    ...
    callbackFromNative: function (callbackId, isSuccess, status, args, keepCallback) {
        try {
            ...
            if (callback) {
                console.debug('received message from '+callbackId,'cordovajs');
            ...
            console.error(msg,'cordovajs');
            err.stack && console.error(err.stack,'cordovajs');
    ...
    addConstructor: function (func) {
        channel.onCordovaReady.subscribe(function () {
            try {
                ...
                console.log('Failed to run constructor: ' + e,'cordovajs');
            }
        });
    }
};
...
function checkArgs(spec, functionName, args, opt_callee) {
    ...
    if (errMsg) {
        ...
        if (typeof jasmine === 'undefined') {
            console.error(errMsg,'cordovajs');
        }
...
var channel = {
    ...
    join: function (h, c) {
        ...
        for (var j = 0; j < len; j++) {
            if (c[j].state === 0) {
                console.error('Can only use join with sticky channels.','cordovajs');
...
Channel.prototype.subscribe = function (eventListenerOrFunction, eventListener) {
    ...
    if (this.type !== 'deviceready') {
        console.debug(this.type,'cordovajs');
    }
...
var iOSExec = function () {
    ...
    if (service !== 'Console') {
        console.debug('post message ' +  callbackId ,'cordovajs')
    }
...
function logUnfiredChannels(arr) {
    for (var i = 0; i < arr.length; ++i) {
        if (arr[i].state !== 2) {
            console.error('Channel not fired: ' + arr[i].type,'cordovajs');
        }
    }
}
...
window.setTimeout(function () {
    if (channel.onDeviceReady.state !== 2) {
        console.error('deviceready has not fired after 5 seconds.','cordovajs');
...
function addEntry(strategy, moduleName, symbolPath, opt_deprecationMessage) {
    if (!(moduleName in moduleMap)) {
        console.error('Module ' + moduleName + ' does not exist.','cordovajs');
